...
export function Calculatie() {
  ...
  const berekenTotalen = () => {
    ...
    // Bereken eerst de totalen per categorie met winst op bruto prijzen
    const totalen = selectedRegels.reduce(
      (acc, regel) => {
        const item = bibliotheekItems.find(i => i.id === regel.regelId);
        if (!item) return acc;

        const brutoPrijs = item.prijs * winstFactor; // Pas winst toe op bruto prijs
    ...
    return {
      ...totalen,
      subtotaal,
      winstRisico: 0, // Winst is al verwerkt in de bruto prijzen
      btw,
    ...
    <div className="container mx-auto p-6">
        ...
        <div className="mb-6">
          ...
          {Object.entries(regelsByHoofdstuk).map(([hoofdstukNaam, hoofdstukRegels], index) => {
            const hoofdstukTotaal = hoofdstukRegels.reduce((sum, regel) => {
              ...
              return sum + (brutoPrijs * regel.aantal);
            }, 0);
                ...
                <table className="min-w-full mb-2">
                  <thead className="bg-red-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-right">Bruto Prijs</th>
                  ...
                  <tbody>
                    {hoofdstukRegels.map((regel) => {
                      ...
                      const brutoPrijs = item.prijs * winstFactor;
                      const totaal = brutoPrijs * regel.aantal;
                      ...
                      return (
                        <tr key={`${hoofdstukNaam}-${regel.regelId}`} className="hover:bg-red-50">
                          ...
                          <td className="px-4 py-2 text-right">€{brutoPrijs.toFixed(2)}</td>
        ...
        <div className="border-t border-red-200 pt-4">
          <div className="w-1/2 ml-auto space-y-2">
            ...
            <div className="flex justify-between font-medium pt-2 border-t border-red-100">
              <span>Subtotaal (incl. winst):</span>
      ...
      {toonOfferte && projectDetails.klantId && (
        <Offerte
          offerte={{
            id: bewerkCalculatie || Date.now().toString(),
            calculatieId: bewerkCalculatie || Date.now().toString(),
            datum: new Date().toISOString(),
            klantId: projectDetails.klantId,
            projectDetails: {
              projectNaam: projectDetails.projectNaam,
              projectLocatie: projectDetails.projectLocatie,
              projectOmschrijving: projectDetails.projectOmschrijving,
              winstRisicoPercentage: projectDetails.winstRisicoPercentage,
              btwPercentage: projectDetails.btwPercentage,
            },
            regels: selectedRegels,
            status: 'concept',
            totalen,
